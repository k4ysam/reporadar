...
class DevpostClient:
    ...
    def _load_robots(self) -> RobotFileParser:
        if self._robots is None:
            ...
            try:
                ...
                # If robots.txt unreachable, assume allowed and proceed politely
                pass
            self._robots = rp
        return self._robots
    ...
    def get(self, url: str) -> requests.Response | None:
        ...
        log_api_call(self._db, self._run_id, "devpost", url, resp.status_code, latency_ms)
        return resp
...
def _parse_software_listing(html: str, limit: int) -> list[dict]:
    ...
    for el in soup.select("a.block-wrapper-link, a.software-entry-link, a[data-software-id]"):
        ...
        out.append({
            "devpost_url": href,
            "project_name": (title_el.get_text(strip=True) if title_el else el.get_text(strip=True))[:200],
...
def _parse_project_page(html: str) -> dict:
    ...
    # GitHub URL — look in side panel and body links
    github_url: str | None = None
    for a in soup.select("a[href*='github.com']"):
        href = a.get("href", "")
        # filter out devpost-internal links and login redirects
        if "github.com" in href and "/login" not in href:
            github_url = href.split("?")[0]
            break

    # Demo URL — typically a "Try it out" link
    ...
    # Description: "Inspiration" / "What it does" sections
    desc_parts: list[str] = []
