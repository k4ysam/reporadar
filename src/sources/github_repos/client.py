...
class GithubClient:
    ...
    def __init__(self, db_conn: sqlite3.Connection, run_id: str, token: str):
        self._conn = db_conn
        self._run_id = run_id
    ...
    def get_stargazers_with_timestamps(
        ...
        for page in range(1, max_pages + 1):
            ...
            latency_ms = int((time.monotonic() - t0) * 1000)
            log_api_call(
                self._conn, self._run_id, "github",
                f"/repos/{full_name}/stargazers", resp.status_code, latency_ms,
            )
